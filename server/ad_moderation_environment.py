...
class AdModerationEnvironment(
    Environment[AdModerationAction, AdModerationObservation, AdModerationState]
):
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        self._task_id = kwargs.get("task_id") or kwargs.get("task") or "spam_detection"

        if self._task_id not in TASKS:
            self._task_id = "spam_detection"
        ...
        return AdModerationObservation(
            text=first_step["observation"],
            task_id=self._task_id,
            ad_content=first_step["observation"],
            severity="medium",
        )

    def step(self, action: AdModerationAction) -> AdModerationObservation:
    ...
    @property
    def state(self) -> AdModerationState:
